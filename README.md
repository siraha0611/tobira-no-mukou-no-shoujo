...
- **おはなしモード**(設定⚙): 彼女が話しおわると自動でマイクが開き、あなたが話しおわるとそのまま届く——ハンズフリーの会話
- **音の風景**(設定⚙): かすかな風と遠い風鈴の環境音。おもいでが進むと、扉に灯る光の粒とともに世界が少しずつ明るくなります
- はじめての方へ向けて、入口に「RPって、なに?」の短い案内があります
- 「RPがはじめてのかたへ」紹介ページ: <https://takuwith-coc.com/rp>

## 🚪 展示モード(インスタレーション)

実物の扉を設置し、「扉のむこうから少女が話しかけてくる」無人展示として運転できます。
URL に `?mode=exhibit` を付けて開き、「展示をはじめる」を押すだけ——
来場者が扉に話しかけると自動で物語がはじまり、終わる(または立ち去る)と自動で次の来場者を待ちます。

→ 機材・設営・運用の詳細: [`docs/展示インスタレーション_設計書.md`](docs/展示インスタレーション_設計書.md)
